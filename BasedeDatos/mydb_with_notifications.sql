...
-- --------------------------------------------------------
--
-- Triggers y Eventos para Notificaciones
--

-- Nota: El trigger notifica a TODOS los usuarios en la lista de espera.
-- Esto podría causar que varios usuarios intenten alquilar una única copia disponible.
-- Una mejora futura podría ser notificar solo al primer usuario de la lista.
DELIMITER $$
CREATE TRIGGER `trg_notify_on_availability` AFTER UPDATE ON `pelicula`
FOR EACH ROW
BEGIN
    -- Si una película no tenía copias y ahora tiene, notificar a la lista de espera.
    IF OLD.ncopias = 0 AND NEW.ncopias > 0 THEN
        INSERT INTO notificaciones (id_usuario, mensaje)
        SELECT 
            le.Usuario_id_usuario,
            CONCAT('¡Buenas noticias! La película "', NEW.titulo, '" que esperabas ya está disponible.')
        FROM lista_espera le
        WHERE le.pelicula_id_pelicula = NEW.id_pelicula;

        -- Opcional: Limpiar la lista de espera para esta película después de notificar.
        -- DELETE FROM lista_espera WHERE pelicula_id_pelicula = NEW.id_pelicula;
    END IF;
END$$ 
DELIMITER ;


-- Evento programado para recordatorios de devolución.
-- Asegúrate de que el planificador de eventos de MySQL esté activado.
-- Puedes activarlo con: SET GLOBAL event_scheduler = ON;

DELIMITER $$
CREATE EVENT `evt_rental_reminder` 
...
BEGIN
    ...
    SELECT 
        ...
        CONCAT('Recordatorio: El alquiler de la película "', pel.titulo, '" vence mañana. ¡No olvides devolverla a tiempo!')
    FROM prestamo p
    JOIN cinta c ON p.cinta_id_cinta = c.id_cinta
    JOIN pelicula pel ON c.pelicula_id_pelicula = pel.id_pelicula
    WHERE p.estado_alquiler = 'en curso' AND DATE(p.fecha_devolucion) = CURDATE() + INTERVAL 1 DAY;
END$$ 
DELIMITER ;
